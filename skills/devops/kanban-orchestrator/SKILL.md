...
## Project bootstrap for a new repo

When the user asks you to "initiate" kanban for a project, treat it as a board/profile bootstrap task, not as generic planning:

1. Create or select a project board for the repo/worktree.
2. Set the board's default workdir to the project root.
3. Create a project-specific orchestrator profile cloned from the active/default profile.
4. Verify the board exists in `hermes kanban boards list` and then use `hermes kanban --board <slug> ...` explicitly for later calls instead of relying on sticky current-board state.
...
2. First turn each lane's execution criteria into a minimal test plan: what should fail before implementation, and what evidence will prove closeout.
3. Then turn those tests into concrete worker tasks that build the functionality needed to make the tests pass.
4. Map each lane to one of the profiles you discovered in Step 0. If a lane doesn't fit any existing profile, ask the user which to use or create.
5. Decide whether each lane is independent or gated by another lane.
6. Create independent lanes as parallel cards with no parent links.
7. Create synthesis/review/integration cards with parent links to the lanes they depend on. A child created with unfinished parents starts in `todo`; the dispatcher promotes it to `ready` only after every parent is done.
