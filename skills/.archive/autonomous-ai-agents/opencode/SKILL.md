...
metadata:
  hermes:
    ...
    related_skills: [claude-code, codex, hermes-agent, coding-agent-tools, multi-agent-evaluation-router]
---
...
7. For low-stakes Hermes auxiliary paths (like context compression), prefer a direct free OpenCode model pin and keep primary chat / approval routing on their existing providers.
8. For Hermes low-stakes auxiliary paths, set `auxiliary.compression.provider` and `auxiliary.compression.model` with `hermes config set`, then verify with `opencode run` smoke output before treating the pin as done.
9. For model-comparison work, fan out to multiple OpenCode agents pinned to different free models and keep scoring / persistence separate from generation.
10. Use a separate rubric and reviewer pass when the goal is to compare outputs; do not let the generator also be the sole judge.
11. See `references/low-stakes-routing-for-hermes.md` for the session-tested pinning pattern and fallback notes.
12. See `references/multi-agent-eval-routing.md` for the split-lane pattern: prompt fan-out, rubric design, competing reviewers, and persisted evaluation signals.
13. See the reusable `multi-agent-evaluation-router` skill for a codified workflow and persistence helper.
14. Verify the diff and tests yourself before telling the user it worked.
