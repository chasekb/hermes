...
`main` is the canonical branch for this repository. The installed upstream source remains `/home/kahlil/.hermes/hermes-agent` from `NousResearch/hermes-agent`; this repository contains configuration and operations policy only.
...
- Source-oriented skill packages are not retained: the Hermes runtime guide, Codex coding lane, mixture-of-agents implementation guide, containerized build verification, and source-branch stash-gap reference were removed.
