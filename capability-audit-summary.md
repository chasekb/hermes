...
Scope: /path/to/hermes/.hermes

## Confirmed state

### Root / global Hermes home
- Hooks are wired in config.yaml to a single router script: /path/to/hermes/.hermes/agent-hooks/hook_router.py.
...
- Direct runtime checks against /path/to/hermes/.hermes/agent-hooks/hook_router.py confirmed that representative events reach the centralized router, pre_llm_call emits the current local/git context, pre_approval_request stays on the auto-accept path, and destructive terminal commands are blocked before execution.
