...
**Sticky default scope pitfall:** `hermes profile use` writes the active profile to `~/.hermes/active_profile` for the current Hermes home, so it is shared by every session using that home. For project-specific behavior without changing the global default, prefer an explicit `hermes -p <profile> ...` invocation or a small project wrapper script; use a separate Hermes home only when you intentionally want isolation of sessions, logs, and local state.
