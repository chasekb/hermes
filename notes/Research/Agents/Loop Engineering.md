...
- OpenAI Agents Python now treats a renewed interruption as eligible to recover a previously failed resumed-session write, including after serialized state round-trips. Approval/resumption loops should persist pending session-write metadata across both retry and interruption transitions, and should prove that already-executed approved tools are not repeated.
- Google ADK now resolves an invocation from every function response in a message rather than only the first. Parallel-tool loops should reject responses spanning multiple invocations, while accepting multiple responses from one invocation, so call/result attribution cannot silently follow message order.
- OpenAI Agents Python exposes streamed transcription language lists and keywords and snapshots the effective transcription configuration for subsequent turns. Voice loops should treat provider request configuration as immutable per turn/session boundary and record whether explicit language lists superseded the legacy single-language field.

## 2026-08-26 review addendum
- OpenAI Agents JS now treats an approved-tool resume with an unacknowledged ordinary Session append as a durable reconciliation state: it claims the live RunState, binds the runtime Session and persistence policy, checkpoints the exact pre-append history, and accepts only the unchanged or exact committed history on retry. The checkpoint survives cancellation/read/append rejection and is cleared only after compaction succeeds; missing/different/server-managed Sessions and concurrent RunState reuse fail closed before further model/tool/guardrail side effects. Resume loops should model append settlement and compaction as explicit states rather than turning uncertain persistence into a normal error-handler result.
- Google ADK now surfaces errors and interrupts from detached fire-and-forget dynamic workflow nodes instead of allowing the workflow to report success. Detached human-input interruptions are converted to errors because the node cannot be resumed, while clean detached completion remains successful. Loop implementations should either await dynamic work or record detached-task outcome, cancellation, and resumability explicitly; tests should cover both in-flight failures and the documented limitation for failures that finish before the outcome check.
- Google ADK's `ParallelAgent` now unwraps a single sub-agent failure from `BaseExceptionGroup` so callers receive the original exception type; multiple failures remain grouped. Parallel loop error handling should preserve branch identity and exception type for single failures while retaining multi-failure aggregation semantics.

## 2026-08-27 review addendum
- Google ADK added an experimental `FallbackModel` that tries an ordered list of models and returns the first successful response without retrying an individual model or routing by cost/task. Loop policies should distinguish model failover from retry, record the attempted model sequence and serving model, and avoid treating fallback as an unbounded retry loop.
- Google ADK workflow failures now preserve upstream status codes and details, including canonical API status values where available, instead of exposing only a Python exception class. Loop error transitions should retain both normalized termination classification and provider status/details for retry, escalation, and replay decisions.
- Google ADK now validates `app_name` before interpolating it into a deployment Dockerfile. Configuration-driven loops should reject unsafe identifiers before generating deployment artifacts or triggering side effects, and retain the validation reason as setup evidence.
- Google ADK's Spanner tool path now releases clients, transports, and sessions created for each tool call. Tool-loop closeout should assert per-call resource ownership and cleanup even when calls fail, rather than relying only on process-level teardown.

## 2026-08-29 review addendum
- Google ADK now matches HITL branch run IDs as parsed path components rather than substring matches, preventing an ID such as `int-1` from resolving a branch carrying `int-10`. Resumption loops should use structured branch/call identity and include collision fixtures in replay tests.
- Google ADK's `LoopAgent` now treats `max_iterations=0` (and negative values) as zero passes while reserving `None` for unlimited execution. Loop configuration and A2A-description tests should distinguish omitted limits from explicit zero limits so termination policy is not silently widened.
- OpenAI Agents JS now preserves pending approval-related input before handoffs and checkpoints filtered handoff input while resumed Session writes are unsettled, then restores it only after the write/compaction transition completes. Approval and handoff loops should treat resumed side effects as a shared persistence boundary and prove that retries do not duplicate approved work or lose filtered input.

## 2026-09-01 review addendum
- Google ADK now stops the current LLM step when preprocessing yields a final function-response event, rather than issuing an unnecessary model call. Loop implementations should treat final preprocessing events as terminal control signals and test that ordinary preprocessing events still reach the model.
- Google ADK reverted persistence of a single-turn workflow node's synthetic input into the stored session while retaining the skip-on-resume behavior. Replay loops should distinguish ephemeral node input from durable user/function-response history and ensure synthetic input cannot shadow persisted responses during resumption.
- Google ADK's concurrent session-create race now has a typed `AlreadyExistsError` outcome for the losing caller. Session loops should make create-if-absent non-atomicity explicit and handle duplicate creation as a recoverable ownership decision, not a provider-specific SQL failure.
- Google ADK's HTTP-client cleanup now uses weak-reference exit hooks and a retained task set for asynchronous close operations. Provider loops should make client lifetime and cleanup task ownership explicit, especially when close is invoked from a running event loop or during process shutdown.
- OpenTelemetry's authoritative positional finish-reason contract means loop termination telemetry must preserve one reason per expected generation and encode missing reasons as `error`; output-message-local finish reasons are no longer the portable control signal.
- SWE-bench's Docker execution helper now closes both the streaming reader and its underlying HTTP response on completion and timeout. Container loops should treat transport closeout as part of the terminal transition, including idempotent cleanup after a timeout has already interrupted the reader.
- SWE-bench's LiteLLM path makes context-window filtering, bounded retries, and strict pricing resolution part of a provider-agnostic inference loop. Benchmark loops should not silently run unknown-context or unknown-price models without recording the skipped safeguard or failing the cost gate.

## 2026-09-02 review addendum
- Google ADK added an optional `transfer_reason` to `transfer_to_agent`, while keeping the parameter out of the default tool declaration; the reason is stored in event actions only when supplied. Delegation loops should keep transfer target, optional rationale, and tool-schema exposure distinct, and should test that compatibility defaults do not silently expand the model-visible control surface.
- Google ADK fixed task-mode follow-up handling so a new user message that borrows an open delegation's invocation ID is processed as a new turn rather than recovered as the original turn's content. Loop harnesses should distinguish borrowed invocation scope from replay/resume provenance and include repeated-message follow-ups in task-mode tests.
- Google ADK's resumable workflow checkpoint now omits `resume_inputs`, rebuilding them from session function responses instead. Authorization/HITL loops should verify that credentials and other sensitive continuation values are not checkpointed while the function-response identity needed for safe resumption remains durable.

## 2026-09-03 review addendum
- Google ADK now checks that an auth-resumed function call was authored by the current agent before executing it. In shared-session multi-agent loops, call identity alone is insufficient: resumption must bind call ID to agent ownership and skip foreign calls rather than executing them with the current agent's tool scope.
- Google ADK refactored function handling into a preparation phase (tool resolution, argument copying, and before-tool callbacks) and an execution phase, with live non-blocking tools launched separately. This makes pre-execution overrides and execution/cleanup boundaries explicit, but the live path currently documents that confirmation state is not threaded through and confirmation-gated tools cannot be resumed there; loop harnesses should test both phase isolation and that limitation.

## 2026-09-04 review addendum
- Google ADK now cancels an invocation when a synchronous `run()` event generator is closed before completion. Sync loop consumers should treat generator close as cancellation, propagate it to in-flight work, and test that no later events or side effects escape after early close.
- Google ADK's inference span now ends at the model's finish reason rather than at the first streamed chunk. Streaming loops should keep span lifetime aligned with semantic completion and test that partial output does not prematurely close inference telemetry.
- Google ADK now treats an error event without function calls as a final response, skips summarization for terminal A2A task states, and closes task-mode delegation when the remote agent finishes. Delegation loops should make terminal-state recognition explicit and avoid extra model/summarization work after a provider or remote task has already ended.

## 2026-09-05 review addendum
- Google ADK now captures the `contextvars` context left by before-tool preparation callbacks and creates each parallel execution task from that snapshot, preserving callback-set authorization/tracing context while isolating sibling calls and the caller. Tool loops should model preparation context as an explicit handoff artifact and test both propagation and per-call isolation.
- OpenAI Agents JS now clears the compaction response ID and last-store state when an OpenAI Responses compaction session is cleared. Compaction loops should invalidate provider continuation state together with session history and reject previous-response compaction until a new response identity is established.
