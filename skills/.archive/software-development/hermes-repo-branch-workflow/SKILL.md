...
### Machine-local branch setup
When you start work on a machine, create a clearly named local branch for that machine before making further edits.
- Prefer a `workflow/...-local` or similar suffix that makes the branch obviously machine-scoped.
- Verify the switch with `git branch --show-current` and `git status --short --branch`.
- If the worktree already contains unrelated changes, keep them isolated and do not assume the new branch is clean.
