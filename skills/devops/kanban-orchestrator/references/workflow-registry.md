...
| survey-first-intake | A new capability surface is proposed | User request, backlog item, existing refs | Survey brief, source list, reuse/adapt/new recommendation | Run the public skill / MCP / workflow survey gate before implementation starts | Pre-flight |
| obsidian-context-layer | Durable project continuity is needed without loading a whole vault | Project index, linked notes, current turn summary | Selective note retrieval plan and write-back summary | Start from the project index, route note operations through `~/.hermes/agent-hooks/hook_router.py`, then update the session summary and decision log | Revision |
| local-rag-profile | A local retrieval workflow is needed with explicit routing rules | Index note, MCP sources, workflow registry | Local-RAG runbook or profile shape | Use the local-rag profile shape and verify there is no hidden fallback | Pre-flight |
